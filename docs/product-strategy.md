...
| Signed typed plugin registry | Offline schema 1 verifier, quorum rotation, rollback floor and SemVer/protocol resolver `bdb2148`; authenticated `pam add` gate `7cf672e`; exact verified-byte Composer source and bounded artifact retention `01f0273`; recoverable project rotation adoption `3bf6d08`; canonical ceremony payload, Ed25519 key identity and operational runbook `c8f9a5f`; signed cross-surface compatibility-matrix export | Descriptor and IDL integrity locks; signed Android runtime installer and provenance `9ddb46d` | Executable protocol/identity/hash checks; signed host acquisition, provenance and bounded retention | Installer enforcement and a machine-readable Server/Native/Desktop dashboard source are implemented; official multi-custodian ceremony, independent fingerprint publication and hosted catalog remain open |
