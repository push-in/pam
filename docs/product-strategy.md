...
| Process manager | [PM2 restart strategies](https://pm2.keymetrics.io/docs/usage/restart-strategies/) and [application declaration](https://pm2.keymetrics.io/docs/usage/application-declaration/) | Automatic crash restart, configurable delay, capped exponential backoff, stable-uptime reset, restart limits, detached logs, and declarative applications | **Shipped:** PAM persists explicit online/stopped intent, defaults to automatic recovery, doubles retry delay to a bounded cap, resets failures after stable uptime, opens a circuit after repeated instability, and exposes integer-coded recovery evidence over JSON and `pam.toml`. **Prove next:** publish recovery-time and crash-loop resource benchmarks. |
...
| Verifiable benchmark evidence | `f37716a`, workflow `0c4a2f7`; controlled overload suite proves bounded `503`/`Retry-After` and post-burst recovery independently from happy-path throughput | Manifest `742e0e4` | Attested package reproducibility `acc3a7c`; authenticated footprint and 5% release-baseline gate `2dad206` | Shipped; public runs depend on CI execution |
