...
PAM Native UI therefore retains real Android emulator and UIKit simulator gates
...
3. Typed Composer package metadata with executable compatibility contracts.
...
| Accessible adaptive design tokens | Desktop starter run-green identity and WCAG/forced-color gate `f58a2ba`; Product workspace publishes a versioned, fail-closed semantic token contract with light/dark integer mode codes, 4/8 spacing, bounded motion and a 48-unit touch floor | Native tokens `b07d09f`; contrast-gated PAM Native UI themes and Studio `2597bd6`; generated Product app validates and adapts the shared token document over framework theme defaults; iOS source CI makes a validated simulator screenshot mandatory; a pinned API 36 workflow builds a clean Product workspace and requires Android light/dark captures | Authenticated worker validates the bounded shared document; renderer validates the response, applies semantic variables and follows system theme changes with a safe fallback; a pinned Servo 1.2.1/Xvfb workflow captures both generated Product themes and decodes them through the host visual harness | Portable token source, adapters, verifier, mandatory iOS capture, and fail-closed Android/Desktop report aggregation are implemented; the first clean hosted certification run remains pending |
...
| Composer-native extension ecosystem | `extra.pam.commands` discovery, package-owned executables, dynamic completion and Composer dry-run compatibility checks | Native SDK, CLI and plugins are ordinary `pushinbr/pam-*` Composer packages | Desktop owns its host lifecycle and commands as `pushinbr/pam-desktop` | Packagist and Composer are the single package registry and resolver; PAM keeps no parallel package allowlist or private registry |
| Flagship cross-surface application | Versioned readiness and idempotent check-in endpoints; deterministic three-surface release index with streaming SHA-256 and create-new evidence | PAM Native UI synchronizes through bounded OkHttp/URLSession and persists check-ins in the SDK's bounded offline queue before transport; the 32-entry flagship snapshot measures 8,222 bytes against Native Storage's 262,144-byte limit | The authenticated PHP worker owns a 32-item/64 KiB private-file outbox and a 24-sample/16 KiB Server history with replacement writes; the operations console exposes exact accessible values and never invents Native health | Live reads, offline mutation replay, bounded local history, and reproducible artifact aggregation span all surfaces with fail-closed integer contracts and portable JSON Schemas; authenticated Native telemetry and platform signing remain explicitly separate |

The ecosystem gate is intentionally conventional: publish packages through
Packagist, express compatibility in Composer constraints, and expose commands
through versioned `extra.pam.commands` metadata. Native and Desktop own their
product-specific downloads and verification; the PAM runtime only discovers
and executes the installed package commands.
