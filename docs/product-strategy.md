...
| Signed typed plugin registry | Offline schema 1 verifier, quorum rotation, rollback floor and SemVer/protocol resolver `6238dfc`; authenticated `pam add` gate `66b18aa`; exact verified-byte Composer source and bounded artifact retention `d49713b` | Descriptor and IDL integrity locks | Executable protocol/identity/hash checks | Composer installer enforcement implemented; official key ceremony, hosted catalog and Native/Desktop binary artifact installers remain open |
