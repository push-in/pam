...
de baixo nível; `pam/api` fornece `Pam\App`, rotas, middleware, error boundary e
descoberta de providers. `pam/socket` fornece a API de eventos/rooms sobre o
transporte WebSocket nativo. A integração PSR e o cliente de testes também vivem
em `pam/psr-bridge` e `pam/testing`.
