...
pub fn render(applications: &[DashboardApplication]) -> String {
    ...
    for app in applications {
        ...
        rows.push_str(&format!(
            "<tr><th scope=\"row\"><span class=\"app-name\">{}</span><span class=\"app-kind\">{}</span></th><td><span class=\"state {state_class}\">{state_label}</span></td><td class=\"number\">{}</td><td class=\"number\">{}</td><td class=\"number\">{}</td><td><span class=\"alert {}\">{}</span></td></tr>",
    ...
    if rows.is_empty() {
        rows.push_str("<tr><td colspan=\"6\" class=\"empty\">No managed applications yet. Start one with <code>pam up … --name api</code>.</td></tr>");
    }
...
const TABLE_START: &str = r##"</strong></div></section><section id="applications" aria-labelledby="applications-title"><h2 id="applications-title">Applications</h2><div class="table-wrap" tabindex="0" role="region" aria-label="Managed applications, horizontally scrollable"><table><caption>Current process state. Status is always written as text and never communicated by color alone.</caption><thead><tr><th scope="col">Application</th><th scope="col">State</th><th scope="col">Workers</th><th scope="col">RSS</th><th scope="col">Tasks</th><th scope="col">Resource signal</th></tr></thead><tbody>"##;
...
mod tests {
    ...
    fn dashboard_is_accessible_bounded_and_escapes_application_names() {
        ...
        assert!(html.contains("Status is always written as text"));
